...
async function loadMixedStudySession(deckId, flashcardLimit = 20, mcqLimit = 10) {
    try {
        ...
        const API_BASE_URL = window.CONFIG ? window.CONFIG.API_BASE_URL : 'http://127.0.0.1:5000';
        const mcqResponse = await fetch(
            `${API_BASE_URL}/mcq/study-session?deck_id=${deckId}&limit=${mcqLimit}`,
            { credentials: 'include' }
        );
...
async function submitMCQAnswer(mcqId) {
    ...
    try {
        const API_BASE_URL = window.CONFIG ? window.CONFIG.API_BASE_URL : 'http://127.0.0.1:5000';
        const response = await fetch(`${API_BASE_URL}/mcq/${mcqId}/check`, {
            method: 'POST',
